...
class MarkdownCleanup:
    ...
    def _normalize_blank_lines(self, markdown: str) -> str:
        ...
        # 2. Standard blank line normalization
        # Replace 3+ newlines with 2 newlines (one blank line)
        result = re.sub(r'\n([ \t]*\n){2,}', '\n\n', result)
        
        # Also clean up trailing whitespace on all lines
    ...
    def _remove_page_footers(self, markdown: str) -> str:
        ...
        def footer_callback(match):
            ...
            for common in common_sections:
                if common.lower() in clean_text.lower():
                    should_remove = True
                    print(f"DEBUG: Removing Footer (Common: {common}): '{clean_text}' '{num}'")
                    break
            ...
            if not should_remove:
                ...
                if len(clean_text) < 50:
                    should_remove = True
                    print(f"DEBUG: Removing Footer (Generic < 50 chars): '{clean_text}' '{num}'")
        ...
        print("DEBUG: Starting Unified Footer Removal")

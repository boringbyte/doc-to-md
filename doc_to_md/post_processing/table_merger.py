...
class TableMerger:
    ...
    def merge_tables(self, markdown: str) -> str:
        """Find and merge split tables in markdown and normalize titles.
        
        Args:
            markdown: The markdown content.
            
        Returns:
            Markdown with merged tables and normalized titles.
        """
        # Step 1: Normalize table titles (headings to bold)
        markdown = self._normalize_table_titles(markdown)
    ...
    def _normalize_table_titles(self, markdown: str) -> str:
        """Normalize table titles (headings) to bold text.
        ...
        **Table 3. Licensed features in iDRAC9**
        """
        # Pattern for headings that start with "Table" (case insensitive)
        # Matches # Table 1, ## Table 1, #Table 1, etc.
        # Captures any leading hashes and the title text.
        pattern = r'^(#{1,6})\s*(Table\s+\d+.*)$'
        
        def replace_callback(match):
            title_text = match.group(2).strip()
            # Clean up potential existing bold markers within the heading
            clean_title = re.sub(r'\*+([^*]+)\*+', r'\1', title_text).strip()
            
            new_title = f"**{clean_title}**"
            logger.debug(f"Normalizing table title: '{match.group(0).strip()}' -> '{new_title}'")
            return new_title

        return re.sub(pattern, replace_callback, markdown, flags=re.MULTILINE | re.IGNORECASE)
